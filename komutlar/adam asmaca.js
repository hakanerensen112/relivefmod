...
var randomkelime = require("rastgelekelime");
let oyndurum = new Set();

module.exports.run = async (bot, message, args) => {

  let kelime = randomkelime({ exactly: 1 })
        ...
        try {
            ...
            while (cevap.length !== confirmation.length && point < 6) {
                await message.channel.send(stripIndents`
                    ${displayText === null ? '**Gelişmiş Adam Asmaca**!' : displayText ? '**Çok iyisin!**' : '**Yanlış Harf!**'}
                         **Kelime:**    \`${display.join(' ')}\`
